<?php
ini_set('include_path', ini_get('include_path') . PATH_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'libs' . DIRECTORY_SEPARATOR . 'PHPExcel' . DIRECTORY_SEPARATOR . 'Classes' . DIRECTORY_SEPARATOR);
include 'PHPExcel.php';
include 'PHPExcel' . DIRECTORY_SEPARATOR . 'Writer' . DIRECTORY_SEPARATOR . 'Excel2007.php';
include realpath(dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'libs' . DIRECTORY_SEPARATOR . 'PHPMailer' . DIRECTORY_SEPARATOR . 'PHPMailerAutoload.php');
...
{
    $query = "SELECT title as team, count(title) as count
              FROM ScheidsApp_zaalwacht Z
              INNER JOIN J3_usergroups G on G.id = Z.team_id
              GROUP BY G.title";
...
{
    $query = "SELECT Z.id, Z.date, G.title as team, Z.wvdw
              FROM ScheidsApp_zaalwacht Z
              LEFT JOIN J3_usergroups G ON G.id = Z.team_id
              WHERE date >= CURDATE() and
              date < DATE_ADD(NOW(), INTERVAL 7 DAY)";
    $stmt = $dbc->prepare($query);
...
function GetMatches($url = "https://api.nevobo.nl/export/sporthal/LDNUN/programma.rss")
{
    ...
    foreach ($xml->channel->item as $i) {
        ...
        $date_stamp = ConvertNevoboDate($match['date_string'] . ", 21:00"); // strptime($match['date_string'] . " " . $year, '%A %e %B %Y');

        $match['date'] = $date_stamp->format("Y-m-d"); //(1900 + $date_stamp['tm_year']) . "-" . sprintf("%02d", 1 + $date_stamp['tm_mon']) . "-" . sprintf("%02d", $date_stamp['tm_mday']);
...
function ConvertNevoboDate($date)
{
    ...
    if (!array_key_exists(strtolower($month), $monthTranslations)) {
                    ...
                    foreach ($userAvailability as $option) {
                        if ($option['date'] == $homeMatch['date'] && $option['time'] == $timeslot) {
                            ...
                            switch ($option['availability']) {
                                case 'Ja':$color = "C6EFCE";
                                    break;
                                case 'Nee':$color = "FFC7CE";
                                    break;
                                case 'Misschien':$color = "FFEB9C";
                                    break;
